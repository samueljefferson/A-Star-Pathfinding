...
class MinHeap:
    ...
    '''
    # breaks g ties in favor of higher g values
    #
    # @param index  the index of the node in the heap
    def breakTiesHigh(self, index):
        #print()
        #print('break high')
        #print()
        if (index < 1):
        ...
        while (index > 0):
            if self.heap[index].g > self.heap[self.parentIndex(index)].g and self.heap[index].f == self.heap[self.parentIndex(index)].f:
                self.heap[index], self.heap[self.parentIndex(index)] = self.heap[self.parentIndex(index)], self.heap[index]
            index = index - 1

    # breaks g ties in favor of lower g values
    #
    # @param index  the index of the node in the heap
    def breakTiesLow(self, index):
        #print()
        #print('break low')
        #print()
        if (index < 1):
