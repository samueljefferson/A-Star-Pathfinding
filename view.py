...
def view_path(path, env):
    ...
    # TODO fix optimizePath()
    #shortest_path = optimizePath(path)
    ...
    #for foot_print in range(0, len(path)):
    foot_print = 0
    pause = True
    show_path = False
    while foot_print < len(path):
        ...
        # TODO ignore this
        #screen = draw_path(shortest_path, grid_info, screen)
        if show_path == True:
        ...
        if best_path == True:
            ...
            # TODO comment this
            #draw_path(path, grid_info, screen)
            ...
            for event in pg.event.get():
                if event.type == pg.KEYDOWN:
                    if event.key == 27:
                        ...
                        pass
                        # TODO renable when optimizedPath() works
                        #show_shortest = True
        '''
    handle_quit()
